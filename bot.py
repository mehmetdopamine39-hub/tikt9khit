...
# Webhook URL (Render'daki site adresin)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bot-status .offline { color: #ff4444; }
...
<body>
    <div class="container">
        ...
        <div class="bot-status">
            🤖 Bot Status: <span class="online" id="botStatus">🟢 ONLINE</span>
            <span style="margin-left:20px;">📨 Hits from Bot: <span id="botHits">0</span></span>
        </div>
        ...
        <div class="controls">
            ...
            <a href="/set_webhook" class="btn btn-bot">🤖 Set Webhook</a>
...
class _Core:
    ...
    def _save_hit_to_db(self, username, email, year, followers, posts, reset_link, from_bot=False):
        try:
            conn = sqlite3.connect(self._db_file)
            c = conn.cursor()
            c.execute('''INSERT INTO hits (username, email, year, followers, posts, reset_link, from_bot)
                         VALUES (?, ?, ?, ?, ?, ?, ?)''',
                      (username, email, year, followers, posts, reset_link, 1 if from_bot else 0))
            conn.commit()
            conn.close()
            if from_bot:
                self._bot_hits += 1
...
''')
    ...
    def _bot_listener(self):
        """Telegram bot'dan gelen mesajları dinle"""
        last_update_id = 0
        while self._running:
            try:
                ...
                if data.get('ok'):
                    for update in data.get('result', []):
                        ...
                        # Hit mesajını parse et
                        if 'HIT' in text or '@' in text:
                            self._parse_bot_hit(text)
        ...
        try:
            ...
            for line in lines:
                if '👤' in line:
                    ...
                    followers_match = re.search(r'(\d+)', line)
                    if followers_match:
                        followers = int(followers_match.group(1))
            
            if username and email:
                # DB'ye kaydet
                self._save_hit_to_db(
                    username=username,
                    email=email,
                    year=year or 'N/A',
                    followers=followers,
                ...
                print(f"{GREEN}[BOT HIT]{RESET} @{username} | {email} | {year}")
                
                # TXT'ye kaydet
                with open(self._hits_file, 'a', encoding='utf-8') as f:
                    f.write(f"BOT: @{username} | {email} | {year}\n")
        except:
            pass
...
@rinexdestek"""
 ...
 ╠══════════════════════════════════════════════╣
 ║  🎯 Total Hits   : {RED}{self._H}{C1}
 ║  🤖 Bot Hits     : {CYAN}{self._bot_hits}{C1}
 ║  🔍 Checked      : {YELLOW}{self._C}{C1}
 ║  ❌ Bad          : {RED}{self._BE + self._BI}{C1}
 ║  📦 Queue        : {CYAN}{qsize}{C1}
 ║  📅 Year         : {yr}{C1}
 ╠══════════════════════════════════════════════╣
...
@app_web.route('/set_webhook')
def set_webhook():
    result = app._set_webhook()
    if result and result.get('ok'):
        return jsonify({'status': 'success', 'data': result})
    return jsonify({'status': 'error', 'data': result})

@app_web.route('/webhook', methods=['POST'])
def webhook():
    """Telegram webhook endpoint - Bot mesajlarını alır"""
...
@rinexdestek"""
            requests.post(
                f"https://api.telegram.org/bot{app._token}/sendMessage",
                json={'chat_id': chat_id, 'text': response}
            )
